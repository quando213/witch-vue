<template>
  <div id="attribute-table">
    <a-button type="primary" icon="plus" id="add-button" @click="createItem">
      Thêm kích cỡ
    </a-button>
    <a-skeleton active v-if="!this.data"/>
    <a-table :columns="columns"
             :data-source="data"
             rowKey="id"
             :pagination="false"
             :scroll="{x: true}"
             v-else
    >
      <span slot="action" slot-scope="action, item">
        <a-button icon="edit" size="small" @click="updateItem(item)"></a-button>
        <a-divider type="vertical"/>
        <a-popconfirm
            :title="`Bạn chắc chắn muốn xoá kích cỡ ${item.name}?`"
            ok-text="OK"
            cancel-text="Huỷ"
            @confirm="deleteItem(item)"
        >
          <a-button icon="delete" size="small" type="danger"></a-button>
        </a-popconfirm>
      </span>
    </a-table>
    <a-modal v-model="visible"
             :title="this.title"
             ok-text="Lưu"
             @ok="onSubmit"
             :after-close="resetForm"
    >
      <a-skeleton active v-if="!this.data"/>
      <a-form-model :model="form"
                    ref="ruleForm"
                    layout="vertical"
                    v-else
                    :rules="rules"
      >
        <a-row :gutter="8">
          <a-col :xs="24" :md="12">
            <a-form-model-item label="Tên" prop="name">
              <a-input v-model="form.name" placeholder="Nhập tên kích cỡ"/>
            </a-form-model-item>
          </a-col>
          <a-col :xs="24" :md="12">
            <a-form-model-item label="Kích cỡ cha" extra="Nếu tạo kích cỡ cha thì không chọn">
              <SizeSelector v-model="form.parent_id" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <a-row :gutter="8">
          <a-col :span="24">
            <a-form-model-item label="Thứ tự">
              <a-input-number v-model="form.sort_number" style="width: 100%;" placeholder="Nhập thứ tự"/>
            </a-form-model-item>
          </a-col>
        </a-row>
      </a-form-model>
    </a-modal>
  </div>
</template>

<script>
import {list, save, store, deleteOne} from "@/modules/products/Attributes/Size/service";
import {assignValuesToForm} from "@/services";
import SizeSelector from "@/components/selectors/SizeSelector";

const columns = [
  {
    title: 'ID',
    dataIndex: 'id'
  },
  {
    title: 'Tên kích cỡ',
    dataIndex: 'name',
  },
  {
    title: 'Thứ tự',
    dataIndex: 'sort_number',
  },
  {
    title: 'Action',
    scopedSlots: {customRender: 'action'},
  },
];

export default {
  components: {SizeSelector},
  data() {
    return {
      data: undefined,
      columns,
      visible: false,
      id: undefined,
      title: '',
      form: {
        name: '',
        parent_id: undefined,
        sort_number: undefined,
      },
      rules: {
        name: [
          {required: true, message: 'Vui lòng nhập tên', trigger: 'change'},
        ],
      },
    };
  },
  async created() {
    await this.updateData();
  },
  methods: {
    async updateData() {
      try {
        const {data} = await list();
        this.data = data.data;
      } catch (e) {
        console.log(e);
      }
    },
    createItem() {
      this.id = undefined;
      this.title = 'Thêm kích cỡ';
      this.visible = true;
    },
    updateItem(item) {
      this.id = item.id;
      this.form = assignValuesToForm(this.form, {
        ...item,
        parent_id: item.parent_id || undefined,
      });
      this.title = 'Chỉnh sửa kích cỡ';
      this.visible = true;
    },
    async deleteItem(item) {
      try {
        await deleteOne(item.id);
        this.$message.success('Xoá kích cỡ thành công');
        await this.updateData();
      } catch (e) {
        console.log(e);
      }
    },
    async onSubmit() {
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          try {
            if (this.id) {
              await save(this.form, this.id);
              this.$message.success('Sửa kích cỡ thành công');
            } else {
              await store(this.form);
              this.$message.success('Thêm kích cỡ thành công');
            }
            await this.updateData();
            this.visible = false;
          } catch (e) {
            this.$message.error(e.response.data.message);
          }
        } else {
          return false;
        }
      });
    },
    resetForm() {
      this.form = {
        name: '',
        parent_id: undefined,
        sort_number: undefined,
      };
    },
  },
}
</script>

<style scoped>
.ant-table td {
  white-space: nowrap;
}

#attribute-table {
  margin: 30px 0;
  text-align: right;
}

#add-button {
  margin-bottom: 10px;
}
</style>